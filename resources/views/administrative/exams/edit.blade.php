...
@section('title', 'تعديل جدول الاختبارات')
...
<div class="form-hero">
    <div style="display: flex; gap: 1.5rem; align-items: center; position: relative; z-index: 2;">
        ...
        <div>
            ...
            <h1 style="font-size: 2rem; font-weight: 900; margin: 0;">تعديل جدول الاختبارات</h1>
        </div>
    </div>
</div>

<div class="container-fluid" x-data="examForm()">
    <form action="{{ route('administrative.exams.update', $exam) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="premium-card mb-4 text-end">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <select name="major_id" class="form-select premium-input @error('major_id') is-invalid @enderror" x-model="majorId" @change="loadLevels()" required>
                                ...
                                @foreach($majors as $major)
                                    <option value="{{ $major->id }}" {{ $exam->major_id == $major->id ? 'selected' : '' }}>{{ $major->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <select name="level_id" class="form-select premium-input @error('level_id') is-invalid @enderror" x-model="levelId" @change="loadSubjects()" required>
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <select name="term_id" class="form-select premium-input @error('term_id') is-invalid @enderror" x-model="termId" required>
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <div class="d-flex gap-3 mt-1">
                                <label>
                                    <input type="radio" name="is_published" value="1" class="d-none" {{ old('is_published', $exam->is_published) == '1' ? 'checked' : '' }}>
                                    <span class="custom-pill-radio"><i class="fa-solid fa-eye"></i> منشور</span>
                                </label>
                                <label>
                                    <input type="radio" name="is_published" value="0" class="d-none" {{ old('is_published', $exam->is_published) == '0' ? 'checked' : '' }}>
            ...
            <div class="col-lg-4">
                <div class="sidebar-actions shadow-sm text-center">
                    <div style="margin-bottom: 2.5rem;">
                        ...
                        <h5 style="font-weight: 900; color: #1e293b;">تحديث البيانات</h5>
                        <p style="color: #64748b; font-size: 0.9rem; font-weight: 500;">عند تحديث هذا الجدول، سيتم تعميم التغييرات فوراً على جميع الطلاب.</p>
                    </div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        <button type="submit" style="width: 100%; padding: 1rem; background: #6366f1; color: white; border: none; border-radius: 14px; font-weight: 800; font-size: 1.1rem; box-shadow: 0 10px 15px -3px rgba(99, 102, 241, 0.3); transition: all 0.3s;" onmouseover="this.style.background='#4f46e5'; this.style.transform='translateY(-2px)'" onmouseout="this.style.background='#6366f1'; this.style.transform='none'" :disabled="items.length === 0">
                            <i class="fa-solid fa-cloud-arrow-up me-2"></i> تحديث الجدول
                        </button>
                        
                        <a href="{{ route('administrative.exams.show', $exam) }}" style="display: block; padding: 1rem; background: #f0f9ff; color: #0ea5e9; border: 1px solid #e0f2fe; border-radius: 14px; text-decoration: none; font-weight: 800; font-size: 0.95rem; transition: all 0.2s;" onmouseover="this.style.background='#e0f2fe'" onmouseout="this.style.background='#f0f9ff'">
                            <i class="fa-solid fa-eye me-1"></i> معاينة الجدول
                        </a>

                        <a href="{{ route('administrative.exams.index') }}" style="display: block; padding: 1rem; background: #f8fafc; color: #64748b; border: 1px solid #e2e8f0; border-radius: 14px; text-decoration: none; font-weight: 800; font-size: 0.95rem; transition: all 0.2s;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='#f8fafc'">إلغاء التعديلات</a>
                    </div>
                </div>
            </div>
        </div>
...
function examForm() {
    return {
        ...
        levels: @json($exam->major->levels),
        subjects: @json($subjects),
        terms: @json($terms),
        @php
            $initialItems = old('items', $exam->items->map(function($i) {
                return [
                    'subject_id' => $i->subject_id,
                    'exam_date' => $i->exam_date->format('Y-m-d'),
                    'start_time' => \Carbon\Carbon::parse($i->start_time)->format('H:i'),
                    'end_time' => \Carbon\Carbon::parse($i->end_time)->format('H:i'),
                    'location' => $i->location
                ];
            })->toArray());
        @endphp
        ...
        async init() {
            // Already initialized with server data
        }
    }
}
</script>
