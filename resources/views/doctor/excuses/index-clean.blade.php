...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h4 mb-1">أعذار الغياب</h1>
            <p class="text-muted mb-0">مراجعة الأعذار الموجهة إليك واتخاذ القرار النهائي لها.</p>
        </div>
    </div>
        ...
        <div class="card-body">
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="search" class="form-control" value="{{ $search }}" placeholder="اسم الطالب أو الرقم الجامعي">
                </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td style="min-width:340px">
                                @if($excuse->status !== 'pending')
                                    ...
                                    @endif
                                @else
                                    <form action="{{ route('doctor.excuses.update', $excuse) }}" method="POST" class="d-grid gap-2">
                                        @csrf
                                        @method('PUT')
                                        <select name="status" class="form-select">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-5 text-muted">لا توجد أعذار مطابقة للفلاتر الحالية.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="card-body border-top">
            {{ $excuses->appends(['status' => $status, 'subject' => $subjectId, 'search' => $search])->links() }}
        </div>
    </div>
</div>
@endsection
