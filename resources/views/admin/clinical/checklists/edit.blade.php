...
@section('title', 'تعديل قائمة تقييم أساسية')
@section('content')
<style>
...
<div class="clinical-page-header">
    <div class="right-side">
        <h1>تعديل القائمة الأساسية ✏️</h1>
        <p>تعديل عناصر وتفاصيل القائمة التقييمية ليتم عكسها كمعيار موحد.</p>
    </div>
    <div class="left-side">
...
<form action="{{ route('admin.clinical.checklists.update', $checklist->id) }}" method="POST" onsubmit="return validateAllMarks(event)">
    @csrf
    @method('PUT')
    ...
    <div class="card-section" style="border: 2px solid #e0e7ff; background: #fafbfe;">
        ...
        <div class="form-row">
            ...
            <div class="form-group" style="grid-column: 1 / -1;">
                ...
                <input type="text" name="description" class="form-control" value="{{ old('description', $checklist->description) }}" placeholder="وصف للتعديلات أو المعيار...">
            </div>
    ...
    <div style="text-align: left; margin-top: 1rem;">
        <button type="submit" class="btn-submit">💾 تحديث القائمة الأساسية</button>
    </div>
</form>
...
<script>
    ...
    // itemsData is already structured as root items with sub_items array from Controller
    const treeItems = @json($itemsData ?? []); 
    ...
    function addSubItem(parentIndex, desc = '', marks = 2) {
        ...
        div.innerHTML = `
            ...
            <input type="text" name="items[${parentIndex}][sub_items][${subIndex}][description]" class="form-control" placeholder="وصف المعيار الفرعي..." value="${desc}" style="font-size:0.85rem; padding:0.5rem;" required>
    ...
    function toggleTimerInput() {
        const type = document.querySelector('input[name="timer_type"]:checked')?.value || (document.getElementById('time_limit_minutes').value ? 'fixed' : 'open');
