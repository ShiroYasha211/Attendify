...
<style>
    ...
    .filter-pill:hover { border-color: #0ea5e9; color: #0ea5e9; }
    ...
    .quiz-card-banner { height: 6px; width: 100%;-background: #0ea5e9; }
...
<div class="quiz-header">
    <div class="quiz-header-content">
        <div>
            <h1><i class="fa-solid fa-clipboard-list me-2"></i>إدارة الكويزات</h1>
            <p>إنشاء وإدارة الاختبارات العامة لجميع الطلاب في النظام</p>
        </div>
        <a href="{{ route('admin.quizzes.create') }}" class="btn-new-quiz">
...
<div class="filter-pills">
    <a href="{{ route('admin.quizzes.index') }}" class="filter-pill {{ $status === 'all' ? 'active' : '' }}">
        <i class="fa-solid fa-layer-group me-1"></i> الكل
    </a>
    <a href="{{ route('admin.quizzes.index', ['status' => 'draft']) }}" class="filter-pill {{ $status === 'draft' ? 'active' : '' }}">
        <i class="fa-solid fa-file-pen me-1"></i> مسودات
    </a>
    <a href="{{ route('admin.quizzes.index', ['status' => 'published']) }}" class="filter-pill {{ $status === 'published' ? 'active' : '' }}">
        <i class="fa-solid fa-globe me-1"></i> منشورة
    </a>
    <a href="{{ route('admin.quizzes.index', ['status' => 'closed']) }}" class="filter-pill {{ $status === 'closed' ? 'active' : '' }}">
...
<div class="quiz-grid">
    ...
    <div class="quiz-card status-{{ $quiz->status }}">
        ...
        <div class="quiz-card-body">
            ...
            <p style="font-size: 0.85rem; color: #64748b; margin-bottom: 0;">{{ $quiz->subject->name ?? 'مسابقة عامة / إدارية' }}</p>

            <div class="quiz-card-info">
                <span class="quiz-info-item">
                    <i class="fa-solid fa-users"></i> {{ $quiz->attempts_count }} طالب
                </span>
                <span class="quiz-info-item">
                    <i class="fa-solid fa-layer-group"></i> {{ $quiz->models_count }} نماذج
                </span>
            </div>
        </div>
        <div class="quiz-card-footer">
            <div style="display: flex; gap: 0.25rem;">
                <a href="{{ route('admin.quizzes.show', $quiz) }}" class="quiz-action-btn">
                    <i class="fa-solid fa-eye"></i> عرض
                </a>
                <a href="{{ route('admin.quizzes.edit', $quiz) }}" class="quiz-action-btn" style="color: #0ea5e9;">
            ...
            <div style="display: flex; gap: 0.25rem;">
                ...
                <form action="{{ route('admin.quizzes.publish', $quiz) }}" method="POST">
                    @csrf @method('PATCH')
                    <button class="quiz-action-btn success"><i class="fa-solid fa-paper-plane"></i></button>
                </form>
                @endif
                <form action="{{ route('admin.quizzes.destroy', $quiz) }}" method="POST" onsubmit="return confirm('حذف الكويز نهائياً؟')">
                    @csrf @method('DELETE')
                    <button class="quiz-action-btn danger"><i class="fa-solid fa-trash-can"></i></button>
                </form>
            </div>
        </div>
    </div>
...
<div class="empty-state">
    ...
    <p class="text-secondary">ابدأ بإنشاء أول كويز إداري من خلال الضغط على الزر أعلاه</p>
</div>
@endif
@endsection
