        ...
        <div class="section-card">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4" x-show="timerMode === 'quiz'">
                    <div class="form-check form-switch m-0">
                        <input class="form-check-input" type="checkbox" name="allow_question_backtracking" value="1" id="allow_question_backtracking" {{ $quiz->allow_question_backtracking ? 'checked' : '' }}>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2 flex-wrap gap-2">
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <div class="d-flex align-items-center gap-1" x-show="use_access_code">
                                ...
                                <input type="text" :name="'models['+mIdx+'][access_code]'" class="form-control-q text-uppercase" style="width: 120px;" x-model="model.access_code" :required="use_access_code" placeholder="مثال: A123" {{ !$canEditContent ? 'disabled' : '' }}>
                            </div>
                        </div>
                        ...
                        <div class="question-card">
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <div class="d-flex gap-2 align-items-center">
                                    <div class="d-flex align-items-center gap-1">
                                        ...
                                        <select :name="'models['+mIdx+'][questions]['+qIdx+'][question_type]'" class="form-select-q py-1 px-2" style="font-size: 0.75rem; border-radius: 8px;" x-model="q.question_type" @change="handleTypeChange(mIdx, qIdx)" {{ !$canEditContent ? 'disabled' : '' }}>
                            ...
                            <template x-for="(opt, oIdx) in q.options" :key="oIdx">
                                <div class="d-flex gap-2 align-items-center mb-1">
                                    ...
                                    <input type="text" :name="'models['+mIdx+'][questions]['+qIdx+'][options]['+oIdx+'][option_text]'" class="form-control-q flex-fill" x-model="opt.text" required {{ !$canEditContent ? 'disabled' : '' }} :readonly="q.question_type === 'true_false'">
                                    <input type="hidden" :name="'models['+mIdx+'][questions]['+qIdx+'][options]['+oIdx+'][is_correct]'" :value="opt.is_correct ? '1' : '0'">
                                    <button type="button" class="btn-remove" @click="removeOption(mIdx, qIdx, oIdx)" x-show="q.options.length > 2 && {{ $canEditContent ? 'true' : 'false' }} && q.question_type !== 'true_false'"><i class="fa-solid fa-times"></i></button>
                                </div>
                            </template>
                            <button type="button" class="btn-add mt-1 py-1 px-2" style="font-size: 0.7rem;" @click="addOption(mIdx, qIdx)" x-show="{{ $canEditContent ? 'true' : 'false' }} && q.question_type !== 'true_false'"><i class="fa-solid fa-plus"></i> إضافة خيار</button>
                        </div>
                    </template>
