...
<div class="container" style="max-width: 100%;" x-data="{ activeTab: 'subjects' }">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <div>
            ...
            <p style="color: var(--text-secondary);">رصد الحضور وعرض التقارير السابقة.</p>
        </div>
    </div>
    ...
    <div x-show="activeTab === 'subjects'" style="margin-top: 1.5rem;">
        ...
        <div class="card" style="text-align: center; padding: 4rem 2rem;">
            ...
            <p style="color: var(--text-secondary);">لم يتم تعيين أي مواد لهذا التخصص والمستوى.</p>
        </div>
        @else
...
<table style="width: 100%; border-collapse: separate; border-spacing: 0;">
                    <thead>
                        <tr style="background: linear-gradient(135deg, #f8fafc, #f1f5f9); text-align: right;">
                            ...
                            <th style="padding: 1rem 1.25rem; border-bottom: 2px solid #e2e8f0; font-weight: 700; color: #475569; font-size: 0.85rem;">المادة الدراسية</th>
                            <th style="padding: 1rem 1.25rem; border-bottom: 2px solid #e2e8f0; font-weight: 700; color: #475569; font-size: 0.85rem;">دكتور المادة</th>
                            <th style="padding: 1rem 1.25rem; border-bottom: 2px solid #e2e8f0; font-weight: 700; color: #475569; font-size: 0.85rem;">الحالة</th>
                            <th style="padding: 1rem 1.25rem; border-bottom: 2px solid #e2e8f0; font-weight: 700; color: #475569; font-size: 0.85rem; text-align: center;">الإجراء</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr style="transition: background 0.15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">
                            ...
                            <td style="padding: 1rem 1.25rem; border-bottom: 1px solid #f1f5f9;">
                                <div style="display: flex; align-items: center; gap: 0.5rem; color: #64748b;">
                                    ...
                                    <span style="font-size: 0.9rem;">{{ $subject->doctor->name ?? 'غير محدد' }}</span>
                                </div>
                            </td>
                            <td style="padding: 1rem 1.25rem; border-bottom: 1px solid #f1f5f9;">
                                @if($subject->allow_delegate_attendance)
                                <span style="display: inline-flex; align-items: center; gap: 4px; font-size: 0.8rem; font-weight: 600; background: #ecfdf5; color: #059669; padding: 5px 12px; border-radius: 20px; border: 1px solid #a7f3d0;">
                                ...
                                @else
                                <span style="display: inline-flex; align-items: center; gap: 4px; font-size: 0.8rem; font-weight: 600; background: #fef2f2; color: #dc2626; padding: 5px 12px; border-radius: 20px; border: 1px solid #fecaca;">
                            ...
                            <td style="padding: 1rem 1.25rem; border-bottom: 1px solid #f1f5f9; text-align: center;">
                                @if($subject->allow_delegate_attendance)
                                <a href="{{ route('delegate.attendance.create', $subject->id) }}"
                                    class="btn btn-primary btn-sm"
                                    style="background: #6366f1; border-color: #6366f1; display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.45rem 1rem; font-size: 0.85rem; font-weight: 600; border-radius: 8px;">
...
<table style="width: 100%; border-collapse: separate; border-spacing: 0;">
                    <thead>
                        <tr style="background: linear-gradient(135deg, #f8fafc, #f1f5f9); text-align: right;">
                            <th style="padding: 1rem; border-bottom: 2px solid #e2e8f0; font-weight: 700; color: #475569; font-size: 0.85rem;">المادة الدراسية</th>
                        ...
                        <tr style="transition: background 0.15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">
                            ...
                            <td style="padding: 1rem; border-bottom: 1px solid #f1f5f9; text-align: center;">
                                <div style="display: flex; justify-content: center; gap: 0.5rem;">
                                    <a href="{{ route('delegate.attendance.create', $session->subject_id) }}?date={{ $session->date->format('Y-m-d') }}&lecture_id={{ $session->lecture_id }}" class="btn btn-secondary btn-sm" title="تعديل السجل">
                                        تعديل
                                    </a>
